(()=>{
'use strict';
const BUILD='world01-story-20260915-3';document.documentElement.dataset.gameBuild=BUILD;
...
addEventListener('keydown',e=>{if(['ArrowLeft','ArrowRight','ArrowUp','ArrowDown',' '].includes(e.key))e.preventDefault();if(mode==='play')bindKey(({KeyA:'a',KeyD:'d',KeyW:'w',KeyS:'s',KeyE:'e',Space:' '})[e.code]||e.key,true);if(e.key==='Escape'&&!e.repeat)togglePause()},{passive:false});addEventListener('keyup',e=>bindKey(({KeyA:'a',KeyD:'d',KeyW:'w',KeyS:'s',KeyE:'e',Space:' '})[e.code]||e.key,false));
$$('[data-key]').forEach(b=>{const key=b.dataset.key;const on=e=>{e.preventDefault();if(mode!=='play')return;b.setPointerCapture?.(e.pointerId);if(key==='jump'){if(!input.jump)input.jumpPressed=true;input.jump=true}else if(key==='action'){if(!input.action)input.actionPressed=true;input.action=true}else input[key]=true;b.classList.add('held')};const off=e=>{e.preventDefault();if(key==='jump')input.jump=false;else if(key==='action')input.action=false;else input[key]=false;b.classList.remove('held')};b.addEventListener('pointerdown',on);['pointerup','pointercancel','lostpointercapture'].forEach(t=>b.addEventListener(t,off))});
...
function startLevel(i,resumeSaved=true){
 if(!window.PBOT_ACTORS.ready||(i===0&&!window.PBOT_WORLD01.ready)){
...
function kill(fromX){const p=state.player;if(p.inv>0||state.deadTimer>0)return;state.lives--;state.damageCount++;p.hurtTime=.28;p.animState='hurt';p.animTime=0;state.shake=10;p.vx=fromX==null?0:(p.x<fromX?-190:190);p.vy=-250;burst(p.x+p.w/2,p.y+p.h/2,12,'#ff5964');updateHud();if(state.lives<=0){state.lives=3;p.cpX=90;toast('Спроба перезапущена')}state.deadTimer=.65;p.inv=1.8}
...
function finish(){if(state.finished||(levelIndex===0&&!window.PBOT_STORY.canFinish()))return;state.finished=true;state.player.animState='finish';state.player.animTime=0;window.PBOT_STORY.completed();const L=state.L;let stars=1;if(state.scoreBeans>=L.beans)stars++;if(state.damageCount===0)stars++;save.unlocked=Math.max(save.unlocked,Math.min(45,levelIndex+2));save.stars[L.id]=Math.max(save.stars[L.id]||0,stars);save.best[L.id]=Math.min(save.best[L.id]||Infinity,state.time);save.beans+=state.scoreBeans;save.tokens+=state.scoreTokens;persist();setTimeout(()=>showComplete(stars),500)}
function showComplete(stars){mode='complete';$('#completeTitle').textContent=state.L.boss?'BROVARY ЗВІЛЬНЕНО':'РІВЕНЬ ПРОЙДЕНО';$('#completeMeta').textContent=levelIndex===0?'Сигнал відновлено. Наступна глава — «Паркові стежки».':`${state.L.id} · ${state.L.world} · ${state.L.name}`;$('#completeStars').textContent='★'.repeat(stars)+'☆'.repeat(3-stars);$('#completeStats').textContent=`☕ ${state.scoreBeans} · ✦ ${state.scoreTokens} · ${state.time.toFixed(1)} c`;$('#completeScreen').classList.remove('hidden')}

function update(dt){if(mode!=='play'||!state||state.finished)return;state.time+=dt;const p=state.player,L=state.L;if(p.inv>0)p.inv-=dt;if(state.shake>0)state.shake=Math.max(0,state.shake-dt*28);if(state.deadTimer>0){state.deadTimer-=dt;window.PBOT_ACTORS.update(p,dt,false);if(state.deadTimer<=0)respawn();return}
 const foot=p.y+p.h;p.crouching=!!input.down&&p.onGround;p.h=p.crouching?Math.round(p.standH*.6):p.standH;p.y=foot-p.h;
 const speed=215+Math.min(5,save.upgrades.speed)*18,accel=1600;const dir=(input.right?1:0)-(input.left?1:0);if(dir)p.facing=dir;p.vx+=dir*accel*dt;p.vx*=Math.pow(.0015,dt);p.vx=Math.max(-speed,Math.min(speed,p.vx));if(input.down&&p.onGround)p.vx*=.75;if(levelIndex===0&&state.quest?.phase>=6&&!dir)p.vx*=Math.pow(.02,dt);
 ...
 if(levelIndex===0)window.PBOT_STORY.update(state,dt);else if(!state.boss&&p.x>L.length-120)finish();window.PBOT_ACTORS.update(p,dt,state.finished);state.camera+=(Math.max(0,Math.min(L.length-W,p.x-310))-state.camera)*(1-Math.pow(.001,dt));updateHud()}
...
function togglePause(){clearInput();if(mode==='play')mode='pause';else if(mode==='pause')mode='play';window.PBOT_STORY.refresh()}
$('#menuBtn').onclick=()=>{if(mode==='play'||mode==='pause'){window.PBOT_STORY.hide();clearInput();mode='menu';$('#gameShell').classList.add('hidden');$('#menuScreen').classList.remove('hidden');renderMenu()}};
$('#resumeBtn').onclick=()=>{if(!state&&save.storyRun){startLevel(0,true);return;}if(state){$('#menuScreen').classList.add('hidden');$('#gameShell').classList.remove('hidden');resize();clearInput();mode='play';window.PBOT_STORY.resume()}};
$('#nextBtn').onclick=()=>{$('#completeScreen').classList.add('hidden');if(levelIndex<44)startLevel(levelIndex+1);else{window.PBOT_STORY.hide();clearInput();mode='menu';$('#gameShell').classList.add('hidden');$('#menuScreen').classList.remove('hidden');renderMenu()}};
$('#mapBtn').onclick=()=>{$('#completeScreen').classList.add('hidden');window.PBOT_STORY.hide();clearInput();mode='menu';$('#gameShell').classList.add('hidden');$('#menuScreen').classList.remove('hidden');renderMenu()};
function renderMenu(){const grid=$('#levelGrid');grid.innerHTML='';LEVELS.forEach((L,i)=>{const b=document.createElement('button');b.className='level-card';if(i>=save.unlocked)b.classList.add('locked');b.innerHTML=`<span class="lvl-icon">${L.icon}</span><b>${L.id}</b><strong>${L.world}</strong><small>${L.name}</small><em>${'★'.repeat(save.stars[L.id]||0)}${'☆'.repeat(3-(save.stars[L.id]||0))}</em>`;b.disabled=i>=save.unlocked;b.onclick=()=>startLevel(i);grid.appendChild(b)});$('#bankBeans').textContent=save.beans;$('#bankTokens').textContent=save.tokens;$('#campaign').textContent=`${Math.min(save.unlocked,45)}/45`;$('#resumeBtn').classList.toggle('hidden',(!state||state.finished)&&!save.storyRun);$('#resumeBtn').textContent='ПРОДОВЖИТИ ІСТОРІЮ';if($('#playFirst'))$('#playFirst').textContent=save.storyRun?'ПРОДОВЖИТИ · 01-01':'ГРАТИ · 01-01';$$('[data-character]').forEach(x=>x.classList.toggle('selected',x.dataset.character===save.selected))}
...
$('#resetBtn').onclick=()=>{if(confirm('Скинути весь прогрес кампанії?')){save=structuredClone(defaultSave);persist();state=null;window.PBOT_STORY.hide();save.storyRun=null;persist();renderMenu()}};
const playFirst=$('#playFirst');if(playFirst)playFirst.onclick=()=>startLevel(0);
if(['localhost','127.0.0.1'].includes(location.hostname))window.__PBOT_QA__={getState:()=>state,getMode:()=>mode,start:startLevel,input,update,resize,get save(){return save}};
renderMenu();
})();